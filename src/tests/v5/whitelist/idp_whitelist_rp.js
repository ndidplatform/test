...
describe('IdP whitelist RP tests', function () {
  ...
  describe('IdP (idp2) create identity (mode 3) (without providing accessor_id) as 2nd IdP (already onboard at idp3)', function () {
    ...
    const mqSendSuccessIdp2ToIdp3CallbackPromise = createEventPromise();
    const mqSendSuccessIdp3ToIdp2CallbackPromise = createEventPromise();
      ...
      nodeCallbackEventEmitter.on('callback', function (callbackData) {
        if (
          ...
          if (callbackData.node_id === 'idp2') {
            if (callbackData.destination_node_id === 'idp3') {
              mqSendSuccessIdp2ToIdp3CallbackPromise.resolve(callbackData);
            }
          } else if (callbackData.node_id === 'idp3') {
            if (callbackData.destination_node_id === 'idp2') {
              mqSendSuccessIdp3ToIdp2CallbackPromise.resolve(callbackData);
            }
          }
        }
      });
    ...
    it('IdP (idp2) should receive message queue send success (To idp3) callback', async function () {
      this.timeout(15000);
      await receiveMessagequeueSendSuccessCallback({
        nodeId: 'idp2',
        requestId,
        mqSendSuccessCallbackPromise: mqSendSuccessIdp2ToIdp3CallbackPromise,
        destinationNodeId: 'idp3',
      });
    });
    ...
    it('1st IdP should create response (accept) successfully', async function () {
      ...
      const response = await idpApi.createResponse('idp3', {
    ...
    it('IdP (idp3) should receive message queue send success (To idp2) callback', async function () {
      this.timeout(15000);
      await receiveMessagequeueSendSuccessCallback({
        nodeId: 'idp3',
        requestId,
        mqSendSuccessCallbackPromise: mqSendSuccessIdp3ToIdp2CallbackPromise,
    ...
    it('After create identity IdP (idp3) that associated with this sid should receive identity notification callback', async function () {
      this.timeout(15000);
      const notificationCreateIdentity = await notificationCreateIdentityPromise.promise;
      //const IdP2notificationCreateIdentity = await notificationCreateIdentityPromise.promise;
      expect(notificationCreateIdentity).to.deep.include({
    ...
    it('Should get relevant IdP nodes by sid successfully', async function () {
      ...
      let idp = responseBody.find((node) => node.node_id === 'idp3');
      expect(idp.ial).to.equal(2.3);
