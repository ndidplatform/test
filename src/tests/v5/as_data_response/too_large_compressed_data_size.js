...
describe('Too large (compressed) AS data size, 1 IdP, 1 AS, mode 3', function () {
  let namespace;
  let identifier;
  ...
  const data = crypto.randomBytes(1024 * 1024 * 4).toString('base64'); // 4194304 bytes in base64 string (>3MB when compressed)
  ...
  it('AS should NOT be able to send data', async function () {
    this.timeout(35000);
    ...
    const responseBody = await response.json();
    expect(responseBody.error.code).to.equal(20086);
  });
