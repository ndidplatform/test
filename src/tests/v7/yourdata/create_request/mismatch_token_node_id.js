...
describe('Mismatch RP node ID (in parameter and token)', function () {
  const rpNodeId = 'rp1';
  const asNodeId = 'as1';
  ...
  it('RP should NOT be able to create a request', async function () {
    ...
    expect(responseBody.error.code).to.equal(20107);
  });
});

describe('Mismatch AS node ID (in parameter and token)', function () {
  const rpNodeId = 'rp1';
  const asNodeId = 'as1';
  ...
  it('RP should NOT be able to create a request', async function () {
    ...
    expect(response.status).to.equal(400);
    const responseBody = await response.json();
    expect(responseBody.error.code).to.equal(20108);
  });
});
