...
describe('Multiple AS data responses', function () {
  const rpNodeId = 'rp1';
  const asNodeId = 'as1';
  ...
  describe('Multiple response with the same data', function () {
    ...
    it('AS should respond with data successfully', async function () {
      this.timeout(20000);
    ...
    it('AS should respond with data again with the same data successfully', async function () {
      this.timeout(20000);
      const response = await yourDataAsApi.sendData('as1', {
        // node_id: asNodeId,
        request_id: requestId,
      ...
      // IMPORTANT: the result may not be deterministic
      ...
      // expect(response.status).to.be.oneOf([204, 400]);

      // if (response.status === 400) {
      //   const responseBody = await response.json();
      //   expect(responseBody.error.code).to.be.oneOf([20114, 20115]);
      // }
    });
  ...
  describe('Multiple response with the different data', function () {
    const rpReferenceId = generateReferenceId();
    ...
    it('AS should NOT be able to respond with data again with the different data', async function () {
      this.timeout(20000);
      const response = await yourDataAsApi.sendData('as1', {
        // node_id: asNodeId,
        request_id: requestId,
      ...
      // IMPORTANT: the result may not be deterministic
      if (responseBody.error.code !== 20116) {
        this.skip();
      }

      expect(responseBody.error.code).to.equal(20116);
    });
