/**
 * IMPORTANT: Set test config/flag OR Code modifications on RP node required.
...
describe('Data decryption key retry request after request timeout', function () {
  ...
  before(async function () {
    ...
    if (!response.ok) {
      throw new Error('unable to set test config');
    }
  ...
  describe('Request timeout after AS respond', function () {
    ...
    // request status callback at RP (yourDataRequestStatus.DATA_DECRYPTION_PENDING), timed_out = true
    ...
    // request status callback at AS (yourDataRequestStatus.DATA_DECRYPTION_PENDING), timed_out = true
  ...
  describe('Retry request for data decryption key', function () {
    ...
    before(async function () {
      ...
      rpEventEmitter.on('callback', function (callbackData) {
        if (
          ...
          if (
            callbackData.status ===
            yourDataDataDecryptionKeyRetryRequestStatus.PENDING
          ) {
            rp_requestStatusPendingPromise.resolve({
              order: rp_statusCallbackOrder++,
              callbackData,
    ...
    it('RP should create a data decryption key retry request successfully', async function () {
      this.timeout(10000);
    ...
    // request status callback at RP (yourDataDataDecryptionKeyRetryRequestStatus.PENDING)
    it('RP should receive retry request pending status', async function () {
      this.timeout(10000);
  ...
  after(async function() {
    // set test config
    const response = await debugApi.setTestConfig('rp1', {
