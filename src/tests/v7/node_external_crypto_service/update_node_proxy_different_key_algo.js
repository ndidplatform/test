...
describe('Update node behind proxy (keys) tests with external crypto service (different key algorithm)', function () {
  ...
  describe("RP, IdP, AS update nodes' keys tests", function () {
    ...
    before(async function () {
      ...
      response = await apiHelpers.getResponseAndBody(
        commonApi.getNodePublicKeys('proxy1', {
          node_id: 'proxy1_rp4',
        })
      );
      ...
      proxy1EventEmitter.on('callback', function (callbackData) {
        if (callbackData.node_id === 'proxy1_idp4') {
          if (
            callbackData.type === 'update_node_result' &&
      ...
      proxy1EventEmitter.on('callback', function (callbackData) {
        if (callbackData.node_id === 'proxy1_as4') {
          if (
            callbackData.type === 'update_node_result' &&
    ...
    it("RP should update node's master key and public key with check_string, signed_check_string, master_signed_check_string successfully", async function () {
      ...
      const response = await nodeApi.updateNode('proxy1', {
        node_id: 'proxy1_rp4',
    ...
    it("RP node's key list should be updated successfully", async function () {
      const response = await commonApi.getNodePublicKeys('proxy1', {
        node_id: 'proxy1_rp4',
      });
      const responseBody = await response.json();
    ...
    it("AS should update node's master key and public key with check_string, signed_check_string, master_signed_check_string successfully", async function () {
        ...
        signed_check_string: cryptoUtils
          .createSignature(
            ...
            Buffer.from(check_string, 'utf8'),
            {
              key: ASPrivKey,
              padding:
                cryptoUtils.signatureAlgorithm.RSASSA_PSS_SHA_256.padding,
            }
          )
          .toString('base64'),
        master_signed_check_string: cryptoUtils
      ...
      kms.setNodeSigningKey(
        'proxy1_as4',
        ASPrivKey,
        cryptoUtils.signatureAlgorithm.RSASSA_PSS_SHA_256
      );
      kms.setNodeSigningMasterKey(
        'proxy1_as4',
        ASMasterPrivKey,
    ...
    it("AS node's master key and public key should be updated successfully", async function () {
      ...
      expect(responseBody.signing_public_key).to.deep.include({
        public_key: ASPubKey,
        algorithm: cryptoUtils.signatureAlgorithm.RSASSA_PSS_SHA_256.name,
    ...
    describe('Create identity tests after update node key', function () {
      ...
      it('Identity should be created successfully', async function () {
        ...
        const idpNode = idpNodes.find(
          (idpNode) => idpNode.node_id === 'proxy1_idp4'
        );
    ...
    after(async function () {
      ...
      originalNodeSigningKey = kms.getOriginalNodeSigningKey('proxy1_rp4');
      originalNodeSigningMasterKey =
        kms.getOriginalNodeSigningMasterKey('proxy1_rp4');
      originalNodeEncryptionKey =
        kms.getOriginalNodeEncryptionKey('proxy1_rp4');

      await nodeApi.updateNode('proxy1', {
        node_id: 'proxy1_rp4',
        reference_id: rpReferenceId,
        callback_url: config.PROXY1_CALLBACK_URL,
      ...
      originalNodeSigningKey = kms.getOriginalNodeSigningKey('proxy1_idp4');
      originalNodeSigningMasterKey =
        kms.getOriginalNodeSigningMasterKey('proxy1_idp4');
      originalNodeEncryptionKey =
        kms.getOriginalNodeEncryptionKey('proxy1_idp4');
      ...
      originalNodeSigningKey = kms.getOriginalNodeSigningKey('proxy1_as4');
      originalNodeSigningMasterKey =
        kms.getOriginalNodeSigningMasterKey('proxy1_as4');
      originalNodeEncryptionKey =
        kms.getOriginalNodeEncryptionKey('proxy1_as4');
