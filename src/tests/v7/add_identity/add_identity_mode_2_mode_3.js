...
describe('Add identity (mode 2,3) tests', function () {
  ...
  it('NDID should add new namespace (test_add_identity and allowed_identifier_count_in_reference_group = 3) successfully', async function () {
    this.timeout(10000);
    ...
    if (alreadyAddedNamespace) {
      const response = await ndidApi.updateNamespace('ndid1', {
        ...
        description:
          'test_add_identity and allowed_identifier_count_in_reference_group = 3',
        allowed_identifier_count_in_reference_group: 3,
      });
  ...
  describe('idp1 create identity request and add identity (mode 3) tests', function () {
    ...
    const idp2AddIdentityRequestResultPromise = createEventPromise();
    const idp1NotificationCreateIdentityPromise = createEventPromise();
    const idp2NotificationCreateIdentityPromise = createEventPromise();
    const idp1NotificationAddIdentityPromise = createEventPromise();
    const idp2NotificationAddIdentityPromise = createEventPromise();
    let accessorId;
    ...
    before(function () {
      idp1EventEmitter.on('callback', function (callbackData) {
        if (
          ...
          callbackData.requestIdAddIdentity === requestId
        ) {
      ...
      idp1EventEmitter.on('identity_notification_callback', function (
        ...
        if (
          ...
          idp1NotificationCreateIdentityPromise.resolve(callbackData);
        } else if (
          callbackData.type === 'identity_modification_notification' &&
          callbackData.reference_group_code === referenceGroupCode &&
          callbackData.action === 'add_identity'
        ) {
          idp1NotificationAddIdentityPromise.resolve(callbackData);
        }
      });
      ...
      idp2EventEmitter.on('callback', function (callbackData) {
        if (
          ...
          idp2AddIdentityRequestResultPromise.resolve(callbackData);
        }
      });
      ...
      idp2EventEmitter.on('identity_notification_callback', function (
        ...
        if (
          ...
          idp2NotificationAddIdentityPromise.resolve(callbackData);
        }
      });
    });
    ...
    it('idp2 should create identity request (mode 2) successfully', async function () {
      ...
      const response = await identityApi.createIdentity('idp2', {
        ...
        identity_list: [
          ...
          { namespace, identifier: identifier2 },
        ],
        accessor_type: 'RSA',
        accessor_public_key: accessorPublicKey,
        //accessor_id,
    ...
    it('After add identity IdP (idp1) that associated with this sid should receive identity notification callback', async function () {
      this.timeout(15000);
      const notificationCreateIdentity = await idp1NotificationCreateIdentityPromise.promise;
    ...
    it('idp1 should add identity that already onboard at idp2 unsuccessfully', async function () {
      this.timeout(10000);

      const response = await identityApi.addIdentity('idp1', {
        namespace,
        identifier,
        reference_id: errorCaseReferenceId,
        callback_url: config.IDP1_CALLBACK_URL,
        identity_list: [
          {
            namespace,
    ...
    it('After add identity IdP (idp1) that associated with this sid should receive identity notification callback', async function () {
      this.timeout(15000);
      const notificationCreateIdentity = await idp2NotificationAddIdentityPromise.promise;
      //const IdP2notificationCreateIdentity = await notificationCreateIdentityPromise.promise;
      expect(notificationCreateIdentity).to.deep.include({
        node_id: 'idp2',
        type: 'identity_modification_notification',
