...
import * as identityApi from '../../../api/v7/identity';
...
describe('RP create request (mode 1) min_idp = 1 to 2 idps and 1st IdP response with an error code and 2nd IdP response with an error', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
...
describe('RP create request (mode 1) min_idp = 1 to 2 idps and 1st IdP response accept and 2nd should not response', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
  const idp2ReferenceId = generateReferenceId();
...
describe('RP create request (mode 1) min_idp = 1 to 2 idps and 1st IdP response reject and 2nd should not response', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
  const idp2ReferenceId = generateReferenceId();
  const rpCloseRequestReferenceId = generateReferenceId();
  ...
  it('Should get request status with rejected status and not closed successfully', async function () {
    this.timeout(10000);
...
describe('RP create request (mode 1) min_idp = 2 to 2 idps and 1st IdP response with an error code and 2nd IdP response with an error', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
  const idp2ReferenceId = generateReferenceId();
  const rpCloseRequestReferenceId = generateReferenceId();
...
describe('RP create request (mode 1) min_idp = 2 to 2 idps and 1st IdP response accept and 2nd IdP response with an error', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
  const idp2ReferenceId = generateReferenceId();
  const rpCloseRequestReferenceId = generateReferenceId();
...
describe('RP create request (mode 1) min_idp = 2 to 2 idps and 1st IdP response reject and 2nd IdP response with an error', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
  const idp2ReferenceId = generateReferenceId();
  const rpCloseRequestReferenceId = generateReferenceId();
...
describe('RP create request (mode 1) min_idp = 2 to 3 idps and 1st and 2nd IdP response error and 3rd IdP should not response', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
  const idp2ReferenceId = generateReferenceId();
...
describe('RP create request (mode 1) min_idp = 2 to 3 idps and 1st and 2nd IdP response accept and 3rd IdP should not response error', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
  const idp2ReferenceId = generateReferenceId();
  const idp3ReferenceId = generateReferenceId();
...
// accept error accept
describe('RP create request (mode 1) min_idp = 2 to 3 idps and 1st and 3rd IdP response accept and 2nd IdP response error', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
  const idp2ReferenceId = generateReferenceId();
  const idp3ReferenceId = generateReferenceId();
...
// error accept error
describe('RP create request (mode 1) min_idp = 2 to 3 idps and 1st and 3rd IdP response error and 2nd IdP response accept', function () {
  const rpReferenceId = generateReferenceId();
  const idpReferenceId = generateReferenceId();
  const idp2ReferenceId = generateReferenceId();
  const idp3ReferenceId = generateReferenceId();
