...
//* This test file for create identity relevant with all IdP for test idp error response
...
describe('Create identity (mode 2) relevant with all IdP for test', function () {
  ...
  describe('IdP (idp3) create identity (mode 2) as 3rd IdP', function () {
    let namespace;
    let identifier;
    ...
    it('Idp (idp3) should create identity request (mode 2) as 3rd IdP successfully', async function () {
      this.timeout(10000);
    ...
    it('Should namespace/identifier (mode 2) relevant all IdP', async function () {
      this.timeout(15000);
      ...
      expect(idpNodes).to.be.an('array').that.to.have.lengthOf(3);
      const idp1 = idpNodes.find((idp) => idp.node_id === 'idp1');
      expect(idp1).to.not.be.undefined;
      const idp2 = idpNodes.find((idp) => idp.node_id === 'idp2');
      expect(idp2).to.not.be.undefined;
...
describe('Create identity (mode 3) relevant with all IdP for test', function () {
  ...
  describe('IdP (idp3) create identity (mode 3) as 3rd IdP', function () {
    let namespace;
    let identifier;
    ...
    it('Idp (idp3) should create identity request (mode 3) as 3rd IdP successfully', async function () {
      this.timeout(10000);
      const response = await identityApi.createIdentity('idp3', {
        reference_id: referenceId,
        callback_url: config.IDP3_CALLBACK_URL,
    ...
    it('Should namespace/identifier (mode 3) relevant all IdP', async function () {
      this.timeout(15000);
      const response = await commonApi.getRelevantIdpNodesBySid('rp1', {
        namespace,
        identifier,
